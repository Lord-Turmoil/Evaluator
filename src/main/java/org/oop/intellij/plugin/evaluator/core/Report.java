...
import org.oop.intellij.plugin.evaluator.api.RulesSearchResponse;
import org.oop.intellij.plugin.evaluator.api.SonarApiImpl;
import org.oop.intellij.plugin.evaluator.common.IdeaUtils;
import org.oop.intellij.plugin.evaluator.common.exceptions.ApiRequestFailedException;
		...
		for (Integer componentFileNumber : componentFileNumbers) {
			...
			while (reportIssues.hasNext()) {
				...
				if (rule == null) {
					...
					ignoreIssueCount++;
					MessageBusManager.publishLogToEDT(project, String.format("Rule[%s] not obtained, issues will be ignored.", issueRuleKey), LogOutput.Level.ERROR);
					continue;
				}
				...
				if (ignoreIssue) {
					ignoreRules.add(issueRuleKey);
					MessageBusManager.publishLogToEDT(project, String.format("Rule[%s] type[%s] Not support yet.", issueRuleKey, rule.getType()), LogOutput.Level.ERROR);
					continue;
				}
